...
def load_pre_reboot_entities(paths, return_per_source=False):
            ...
            if path.name == "omega_device_registry.json":
                ...
                invalid_keys = (
                    [
                        k
                        for k in data.keys()
                        if k not in ("sensors", "lights", "rooms", "devices", "data")
                    ]
                    if isinstance(data, dict)
                    else []
                )
    ...
    pipeline_trace.append(
        {"step": "after_dict_check", "count": len(filtered), "reason": "must be dict"}
    )
    # Example: filter by entity_id presence
    filtered2 = [e for e in filtered if e.get("entity_id")]
    pipeline_trace.append(
        {
            "step": "after_entity_id_check",
            "count": len(filtered2),
            "reason": "must have entity_id",
        }
    )
    # Example: filter by is_excluded_entity
    from addon.src.utils.registry import is_excluded_entity

    filtered3 = [e for e in filtered2 if not is_excluded_entity(e["entity_id"])]
    pipeline_trace.append(
        {
            "step": "after_is_excluded_entity",
            "count": len(filtered3),
            "reason": "not excluded by is_excluded_entity",
        }
    )
    ...
    # Emit pipeline trace
    with open(
        "output/migration_diagnostics/pre_reboot_entity_pipeline_trace.json", "w"
    ) as f:
        json.dump(pipeline_trace, f, indent=2)
    # Emit canonical id issues
    with open(
        "output/migration_diagnostics/pre_reboot_canonical_id_issues.json", "w"
    ) as f:
    ...
    for src, stat in canonical_id_stats.items():
        print(
            f"[INFO] {src}: with_canonical_id={stat['with_canonical_id']}, without_canonical_id={stat['without_canonical_id']}"
        )
