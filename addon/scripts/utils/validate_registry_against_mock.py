...
def main():
    if len(sys.argv) != 3:
        print("Usage: validate_registry_against_mock.py --output <output.json> --mock <mock.json>")
        sys.exit(2)
    ...
    for i, (o, m) in enumerate(zip(output, mock)):
        ...
        missing = null_or_missing_critical(o, [
            'entity_id','domain','platform','device_class','entity_category','name','area_id','floor_id','device_id','entry_id','integration','join_confidence','join_origin'])
        if missing:
