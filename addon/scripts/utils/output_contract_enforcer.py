...
def log_failure(msg: str, contract_path: str):
    ...
    log_path = os.path.join(log_dir, f'CONTRACT-ENFORCEMENT-FAILURES-{datetime.datetime.now().strftime("%Y%m%dT%H%M%S")}.log')
...
def enforce_contract(contract_path: str):
    ...
    for output in contract.get('outputs', []):
        ...
        if output.get('must_contain', False) and not check_file_not_empty(path):
            ...
            all_passed = False
        if 'min_entries' in output and not check_min_entries(path, output['min_entries']):
            log_failure(f'File {path} has fewer than {output["min_entries"]} entries', contract_path)
            all_passed = False
        if 'required_keys' in output and not check_required_keys(path, output['required_keys']):
            log_failure(f'File {path} missing required keys: {output["required_keys"]}', contract_path)
            all_passed = False
    if not all_passed:
        sys.exit(1)
    ...
    for i, (o, m) in enumerate(zip(out, ref)):
        ...
        for k in o:
            if k in m and type(o[k]) != type(m[k]):
                print(f"[MOCK DIFF] Entity {i}: Type mismatch for {k}: {type(o[k]).__name__} vs {type(m[k]).__name__}")
        if 'join_confidence' in o and not (0.75 <= o['join_confidence'] <= 1.0):
            print(f"[MOCK DIFF] Entity {i}: join_confidence out of range: {o['join_confidence']}")
        missing = [k for k in ['entity_id','domain','platform','device_class','entity_category','name','area_id','floor_id','device_id','entry_id','integration','join_confidence','join_origin'] if k not in o or o[k] is None]
        if missing:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--reference_snapshot', help='Path to mock reference snapshot for diff')
    args = parser.parse_args()
    if not args.contract:
        print('Usage: python output_contract_enforcer.py <contract.yaml> [--reference_snapshot <mock.json>]')
        sys.exit(2)
    ...
    for output in contract.get('outputs', []):
        if output['path'].endswith('omega_registry_master.json'):
            if not args.reference_snapshot:
                print('Error: --reference_snapshot is required when validating omega_registry_master.json')
                sys.exit(2)
