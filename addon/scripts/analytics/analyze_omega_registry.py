...
def parse_args():
    ...
    parser.add_argument(
        "--input", required=True, help="Path to omega_registry_master.json"
    )
    parser.add_argument(
        "--log", required=True, help="Path to cumulative analytics log file"
    )
...
def main():
    ...
    join_conf_stats = {
        ...
        "mean": sum(join_conf_numeric) / len(join_conf_numeric)
    ...
    block = {
        ...
        "connections_breakdown": analyze_connections_breakdown(
            data
        ),  # PATCH: add to output
    }
    ...
    entity_registry_path = str(
        canonical_path("registry_inputs", "core.entity_registry")
    )
