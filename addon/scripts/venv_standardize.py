...
def make_dotvenv(symlink=False):
    ...
    if symlink:
        ...
        print('Creating .venv as a small marker to reuse venv; this is conservative so we will symlink by default')
        dotvenv.symlink_to(repo_venv, target_is_directory=True)
    ...
    try:
        ...
        if 'python.defaultInterpreterPath' in data:
            ...
            for line in data.splitlines():
                if 'python.defaultInterpreterPath' in line:
                    indent = line.split('"')[0]
                    out.append(f'{indent}"python.defaultInterpreterPath": "{new_path}",')
                else:
                    out.append(line)
            ...
            print('No python.defaultInterpreterPath found in workspace — skipping')
            return 1
