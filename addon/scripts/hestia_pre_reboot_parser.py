...
OUTPUT_CONTRACT_PATH = str(canonical_path('support', 'contracts', 'hestia_pre_reboot_parser.output_contract.yaml'))

def parse_legacy_hestia_registries(input_dir, output_summary_path, debug_log=None):
    """
    ...
    debug_entries = []
    for file in files:
        file_debug = {'filename': str(file), 'entry_count': 0, 'sample_entries': [], 'skipped': [], 'malformed': []}
        with open(file) as f:
            try:
                data = json.load(f)
            except Exception as e:
                file_debug['malformed'].append({'error': 'invalid json', 'exception': str(e)})
        ...
        if isinstance(data, list):
            entries = data
        elif isinstance(data, dict) and 'data' in data and 'entities' in data['data']:
            entries = data['data']['entities']
        elif isinstance(data, dict):
            entries = list(data.values())  # fallback, in case it's a dict of entity_id: entity_obj
        else:
            entries = []
    ...
    for field, stats in field_stats.items():
        ...
        source_files = stats['source_files'] if isinstance(stats['source_files'], set) else set()
        value_types = stats['value_types'] if isinstance(stats['value_types'], set) else set()
        example_values = stats['example_values'] if isinstance(stats['example_values'], set) else set()
        summary.append({
            'field_name': field,
            'source_files': sorted(list(source_files)),
            'value_types': sorted(list(value_types)),
            'occurrences': stats['occurrences'] if isinstance(stats['occurrences'], int) else 0,
            'example_values': sorted(list(example_values)),
            'match_to': 'canonical' if field in canonical_fields else 'non-canonical',
        })
    ...
    with open('PATCH-ROUNDTRIP-AUDIT-V2.log', 'a') as audit:
        audit.write('[PATCH-ACTIVE: DEBUG_LOGGER_EXTENSION_V1] Ran parse_legacy_hestia_registries with debug_log=%s\n' % (debug_log or 'None'))

def extract_hestia_pre_reboot_insights(input_dir, out_map, out_hints, out_diag, out_drift, debug_log=None):
    """
    ...
    """
    import re
    files = sorted(Path(input_dir).glob("*.json"), key=lambda f: (
        re.search(r"(\\d{2}-\\d{2})", f.name), f.name), reverse=True)
    ...
    entity_by_id = {}
    entity_file_info = {}
    joinable_fields = ["area_id", "device_id", "mac", "name", "platform", "domain", "device_class"]
    ...
    for file in files:
        ...
        with open(file) as f:
            try:
                ...
                diagnostics.append({'file': str(file), 'error': 'invalid json', 'exception': str(e)})
                file_debug['malformed'].append({'error': 'invalid json', 'exception': str(e)})
                debug_entries.append(file_debug)
                continue
        ...
        for i, (entry, line_hint) in enumerate(all_entity_dicts):
            eid = entry.get('id')
            if eid is None:
                file_debug['skipped'].append({'reason': 'missing id', 'entry': entry})
                continue
            ...
            if eid in entity_by_id:
                ...
                if prev != entry:
                    file_debug['collisions'].append({'entity_id': eid, 'prev_file': entity_file_info[eid]['file'], 'curr_file': str(file)})
            ...
            unexpected = set(entry.keys()) - required_fields - set(joinable_fields)
            ...
            # Diagnostics
            if missing:
                file_debug['missing_required'].append({'entity_id': eid, 'missing': list(missing)})
            file_debug['valid_entries'] += 1
            if len(file_debug['sample_entries']) < 2:
                file_debug['sample_entries'].append({
                    'keys': list(entry.keys()),
                    'types': {k: type(v).__name__ for k, v in entry.items()}
                })
        file_debug['entry_count'] = len(all_entity_dicts)
        debug_entries.append(file_debug)
    # Emit outputs
    with open(out_map, 'w') as f:
        json.dump({k: sorted(list(v)) for k, v in field_map.items()}, f, indent=2)
    with open(out_hints, 'w') as f:
        json.dump(list(entity_by_id[eid] and {'entity_id': eid, 'join_fields': {k: entity_by_id[eid][k] for k in joinable_fields if k in entity_by_id[eid]}} for eid in entity_by_id), f, indent=2)
    ...
    # Log patch execution
    with open('PATCH-ROUNDTRIP-AUDIT-V2.log', 'a') as audit:
        audit.write('[PATCH-ACTIVE: HESTIA_JOIN_GRAPH_FIX_V2] Ran extract_hestia_pre_reboot_insights with deduplication, join field extraction, schema drift enrichment, and enhanced diagnostics. debug_log=%s\n' % (debug_log or 'None'))
    with open(str(canonical_path('logs', 'copilot_patchlog.log')), 'a') as patchlog:
        patchlog.write('[PATCH: HESTIA_JOIN_GRAPH_FIX_V2] join_hints now emit deduped, rich join_fields per entity. %s\n' % (debug_log or ''))
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Hestia Pre-Reboot Registry Analysis (now with --debug_log for .jsonl diagnostics)")
    parser.add_argument('--mode', choices=['enrichment_summary', 'join_graph'], required=True)
    parser.add_argument('--input_dir', default=str(canonical_path('derived_views', 'hestia_registries', 'pre-reboot')) + os.sep, help='Input directory of legacy registries')
    ...
    parser.add_argument('--out_hints', help='Join hints output (for join graph)')
    parser.add_argument('--out_diag', help='Diagnostics log output (for join graph)')
    parser.add_argument('--out_drift', help='Schema drift output (for join graph)')
    parser.add_argument('--debug_log', help='Emit .jsonl debug log of all parsed files and entries (for both modes)')
    args = parser.parse_args()
    if args.mode == 'enrichment_summary':
        output = args.output or str(canonical_path('diagnostics', 'legacy_registry_enrichment_summary.yaml'))
        parse_legacy_hestia_registries(args.input_dir, output, debug_log=args.debug_log)
    elif args.mode == 'join_graph':
        extract_hestia_pre_reboot_insights(
            args.input_dir,
            args.out_map or str(canonical_path('logs', 'hestia_pre_reboot_field_map.json')),
            args.out_hints or str(canonical_path('joins', 'pre_reboot_field_join_hints.json')),
            args.out_diag or str(canonical_path('logs', 'hestia_pre_reboot_field_diagnostics.log')),
            args.out_drift or str(canonical_path('logs', 'hestia_pre_reboot_schema_drift.json')),
            args.debug_log
        )
