...
from utils.join_utils import get_device, get_area, get_floor, get_config, get_restore, is_exposed, extract_connection_fields
...
def main():
    parser = argparse.ArgumentParser(description="Generate omega_registry_master.json with debug log support.")
    parser.add_argument('--output', required=True, help='Output path for omega_registry_master.json')
    parser.add_argument('--debug_log', help='Path to emit debug .jsonl log')
    args = parser.parse_args()

    # --- BEGIN PATCH: Source Contract Enumeration ---
    contract_path = str(canonical_path('support', 'contracts', 'join_contract.yaml'))
    ...
    for fname in all_input_files:
        ...
        fpath = fpath_json if os.path.exists(fpath_json) else os.path.join(input_dir, fname)
    ...
    _ = load_json_with_extract(str(canonical_path('enrichment_sources', 'ha_registries', 'post-reboot', 'core.entity_registry')))
    ...
    for fname in sorted(list(expected_files)):
        fpath_json = os.path.join(input_dir, fname + '.json')
        fpath = fpath_json if os.path.exists(fpath_json) else os.path.join(input_dir, fname)
        status = {'present': os.path.exists(fpath), 'extraction': None, 'reason': None}
        if status['present']:
            try:
    ...
    for e in entity_registry:
        ...
        try:
            ...
            if device:
                device = extract_connection_fields(device)  # PATCH: surface mac/upnp/etc.
            ...
            if device_id and device:
                ...
                if device.get('area_id'):
                    field_provenance['area_id'] = 'from_device → core.device_registry'
            if area_id and area:
                field_provenance['area_id'] = 'core.area_registry'
                if area.get('floor_id'):
                    field_provenance['floor_id'] = 'from_area → core.area_registry'
            if config:
                field_provenance['integration'] = 'core.config_entries'
            if restore:
                field_provenance['state_snapshot'] = 'core.restore_state'
            if exposed:
                field_provenance['exposed_to_assistant'] = 'homeassistant.exposed_entities'
            # --- END PATCH ---
            ...
            if unique_id:
                field_provenance['unique_id'] = 'core.entity_registry' if e.get('unique_id') else 'core.device_registry:identifiers'
            via_device_id, via_path = resolve_via_device_id(device, device_by_id)
            if via_device_id:
                field_provenance['via_device_id'] = 'core.device_registry' + (f' (multi-hop: {via_path})' if via_path else '')
            # --- END PATCH ---
            ...
            joined = {
                'entity_id': entity_id,
                'unique_id': unique_id,
                'domain': e.get('platform') or (entity_id.split('.')[0] if entity_id else None),
                ...
                'labels': e.get('labels') or (device.get('labels') if device else None),
            ...
            joined = {k: v for k, v in joined.items() if v is not None or k in ['entity_id','domain','platform','device_class','entity_category','name','area_id','floor_id','device_id','entry_id','integration','join_confidence','join_origin','enrichment_depth','field_inheritance']}
            ...
            if joined.get('entity_id') and joined.get('domain'):
                ...
                enrichment_traces.append({
                    ...
                    'missing': [src for src in ['core.config_entries','core.restore_state','homeassistant.exposed_entities'] if src not in join_origin]
                })
                ...
                # Track used files
                used_files.update([src for src in join_origin if src in expected_files])
        except Exception as ex:
            print(f"[WARN] Skipped entity (malformed): {repr(e)[:80]}: {ex}")
            continue
    # --- BEGIN PATCH: Unused Files ---
    unused_files = [f for f in loaded_files if f not in used_files and loaded_files[f]]
    ...
    with open(str(canonical_path('logs', 'scratch', 'enrichment_trace_omega_registry.jsonl')), 'w') as f:
        for trace in enrichment_traces:
            f.write(json.dumps(trace) + '\n')
    # Emit propagation audit trace
    with open(str(canonical_path('logs', 'scratch', 'propagation_audit_trace.jsonl')), 'w') as f:
        for rec in propagation_audit:
            f.write(json.dumps(rec) + '\n')
    # Emit source ingestion status
    with open(str(canonical_path('logs', 'scratch', 'source_file_ingestion_status.json')), 'w') as f:
        json.dump(ingestion_status, f, indent=2)
    # Emit unused sources log
    with open(str(canonical_path('logs', 'scratch', 'omega_registry_unused_sources.log')), 'w') as f:
    ...
    print(f"[INFO] Emitted enrichment trace log: {str(canonical_path('logs','scratch','enrichment_trace_omega_registry.jsonl'))}")
    print(f"[INFO] Emitted propagation audit trace: {str(canonical_path('logs','scratch','propagation_audit_trace.jsonl'))}")
    print(f"[INFO] Emitted source ingestion status: {str(canonical_path('logs','scratch','source_file_ingestion_status.json'))}")
    print(f"[INFO] Emitted unused sources log: {str(canonical_path('logs','scratch','omega_registry_unused_sources.log'))}")
