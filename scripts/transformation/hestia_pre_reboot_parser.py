#!/usr/bin/env python3
"""
Run as module: python -m scripts.transformation.hestia_pre_reboot_parser
PATCH-HESTIA-PRE-REBOOT-PARSER-HEADER-V1
Parses legacy Hestia pre-reboot registries and emits enrichment and diagnostics
outputs as per output contract.
"""
...
OUTPUT_CONTRACT_PATH = os.path.join(
    ...
    "../canonical/support/contracts/hestia_pre_reboot_parser.output_contract.yaml",
)

# CONTRACT-DRIVEN: Load joinable fields, required fields, and reference format from contract
with open(
    os.path.join(
        os.path.dirname(__file__), "../canonical/support/contracts/join_contract.yaml"
    )
) as f:
...
joinable_fields = (
    join_contract.get("reference_format", {})
    .get("container_reference", {})
    .get(
        "applies_to",
        ["area_id", "device_id", "mac", "name", "platform", "domain", "device_class"],
    )
)
required_fields = set(
    join_contract.get("reference_format", {})
    .get("container_reference", {})
    .get("required_fields", ["id", "area_id", "name"])
)
...
def parse_legacy_hestia_registries(input_dir, output_summary_path, debug_log=None):
    ...
    for field, stats in field_stats.items():
        ...
        source_files = (
            stats["source_files"] if isinstance(stats["source_files"], set) else set()
        )
        value_types = (
            stats["value_types"] if isinstance(stats["value_types"], set) else set()
        )
        example_values = (
        ...
        summary.append(
            {
                ...
                "occurrences": (
                    stats["occurrences"] if isinstance(stats["occurrences"], int) else 0
                ),
                "example_values": sorted(list(example_values)),
                "match_to": (
                    "canonical" if field in canonical_fields else "non-canonical"
                ),
            }
        )
...
def extract_hestia_pre_reboot_insights(
    ...
    entity_file_info = {}
    file_entity_ids = {}
    # Use contract-driven joinable_fields and required_fields
    for file in files:
        file_debug = {
            "filename": str(file),
            "entry_count": 0,
        ...
        with open(file) as f:
            try:
                ...
                diagnostics.append(
                    {"file": str(file), "error": "invalid json", "exception": str(e)}
                )
                file_debug["malformed"].append(
                    {"error": "invalid json", "exception": str(e)}
                )
        ...
        for i, (entry, line_hint) in enumerate(all_entity_dicts):
            ...
            # Join field extraction (contract-driven)
    ...
    with open(
        ...
        log.write(
            f"[{datetime.datetime.utcnow().isoformat()}] Refactored hestia_pre_reboot_parser.py for contract-driven joinable fields, required fields, reference format, and provenance.\n"
        )
    # Log patch execution
    with open("PATCH-ROUNDTRIP-AUDIT-V2.log", "a") as audit:
        audit.write(
    ...
    with open("canonical/logs/copilot_patchlog.log", "a") as patchlog:
        patchlog.write(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_dir",
        default="canonical/derived_views/hestia_registries/pre-reboot/",
    ...
    if args.mode == "enrichment_summary":
        output = (
            args.output
            or "canonical/diagnostics/legacy_registry_enrichment_summary.yaml"
        )
        ...
        extract_hestia_pre_reboot_insights(
            args.input_dir,
            args.out_map or "canonical/logs/hestia_pre_reboot_field_map.json",
            args.out_hints or "canonical/joins/pre_reboot_field_join_hints.json",
            args.out_diag or "canonical/logs/hestia_pre_reboot_field_diagnostics.log",
            args.out_drift or "canonical/logs/hestia_pre_reboot_schema_drift.json",
            args.debug_log,
        )
